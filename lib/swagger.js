...
function parseSchema(obj) {
  ...
  const spl = obj.name.split('.');
  const modelName = obj.modelName || spl[0];
  if ((spl.length > 1 && spl[1] === 'model') || obj.modelName) {
    return {
...
function parseReturn(tags) {
  ...
  for (let i in tags) {
    if (tags[i]['title'] === 'returns' || tags[i]['title'] === 'return') {
      ...
      rets[key] = {
        description: formatterDescriptions(description[1].trim()),
        headers: headers[key]
      };
...
function parseTypedef(tags) {
  ...
  for (let i = 1; i < tags.length; i++) {
    ...
    if (tag.title === 'property') {
      ...
      var schema = parseSchema(propType);
      if (schema) {
...
function fileFormat(comments) {
  ...
  for (let i in comments) {
    ...
    if (i === 'tags') {
      if (comment.length > 0 && comment[0]['title'] && comment[0]['title'] === 'typedef') {
        const typedefParsed = parseTypedef(comment);
        definitions[typedefParsed.typeName] = typedefParsed.details;
        continue;
      }
      for (let j in comment) {
        let title = comment[j]['title'];
        let parameterRef = route ? parameters[route.uri] : null;
        if (title === 'route') {
          route = parseRoute(comment[j]['description']);
          let tag = parseTag(comment);
          ...
          parameterRef[route.method]['tags'] = [tag[0].trim()];
          tags.push({
            name: typeof tag[0] === 'string' ? tag[0].trim() : '',
            description: formatterDescriptions(typeof tag[1] === 'string' ? tag[1].trim() : '')
          });
        }
        if (title === 'param') {
          let field = parseField(comment[j]['name']),
            properties = {
              name: field.name,
              in: field.parameter_type,
              description: formatterDescriptions(comment[j]['description']),
              required: field.required
            },
            schema = parseSchema(comment[j]['type']);
          // we only want a type if there is no referenced schema
          if (!schema) {
            properties.type = parseType(comment[j]['type']);
          } else {
        ...
        if (title === 'operationId' && route) {
          parameterRef[route.method]['operationId'] = comment[j]['description'];
        }

        if (title === 'summary' && route) {
          parameterRef[route.method]['summary'] = formatterDescriptions(comment[j]['description']);
        }

        if (title === 'produces' && route) {
          parameterRef[route.method]['produces'] = parseProduces(comment[j]['description']);
        }

        if (title === 'consumes' && route) {
          parameterRef[route.method]['consumes'] = parseConsumes(comment[j]['description']);
        }

        if (title === 'security' && route) {
          parameterRef[route.method]['security'] = parseSecurity(comment[j]['description']);
        }

        if (route) {
          parameterRef[route.method]['parameters'] = params;
          parameterRef[route.method]['responses'] = parseReturn(comment);
        }
      }
    }
  }
 ...
 * @returns {Function} To set Swagger api-doc in your express app
